...
import { ModifiedTaskBase, PopulatedContributorsIds } from '../../../../../interfaces/models';
import { RootState } from '../../../../../store/store';

interface RenderRepositoryTasksProps {
    tasks: ModifiedTaskBase[];
    setTasks: React.Dispatch<React.SetStateAction<ModifiedTaskBase[]>>;
    render: string;
    nameFilter: string;
    handleClose: () => void;
}
...
export const RenderRepositoryTasks: React.FC<RenderRepositoryTasksProps> = ({ tasks, setTasks, render, nameFilter, handleClose }) => {
    ...
    const TaskCollaborators = ({ collaborators } : { collaborators: PopulatedContributorsIds[]}) => {
    ...
    const onSubmit = (reasonsTextField: string) => {
      setOpenDialog(false)
    ...
    const InfoDialog: React.FC<InfoDialogProps> = memo(({ infoDialogOpen, setInfoDialogOpen, description, goals}) => {
      return (
        <Dialog
        open={infoDialogOpen}
        onClose={setInfoDialogOpen}
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setInfoDialogOpen()} autoFocus>
            Close
          </Button>
        </DialogActions>
      </Dialog>
                            ...
                            : (
                                <>
                                  <button 
                                    ...
                                    <IosClose className="w-4 h-4" onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined} />
                                  ...
                                  <button
                                    ...
                                    <CheckTwotone className="w-3 h-3" onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined} />
                                  </button>
                                </>
                              )
                          }
                        ...
                        >
                          {
                              ...
                              : <TaskRemove onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined} />
                          }
                      </Icon>
                    </div>
              ...
              <div className="flex justify-between mt-1">
                  <div className='flex space-x-2'>
                    ...
                    }
                    {
                      ...
                      ? null
                      : task.assigned_to && !task.contributors.some(contri => contri._id !== task.assigned_to)
                      ? null
                      : !task.assigned_to && task.contributors.length === 0 
                  ...
                  <InfoDialog
                        infoDialogOpen={openDialogs[task._id] as boolean}
