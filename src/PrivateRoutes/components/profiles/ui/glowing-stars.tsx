...
export const GlowingStarsDescription = ({
  ...
  return (
    <p className={cn("text-base text-white w-[90%] line-clamp-2", className)}>
      {children}
    </p>
  );
};
...
export const GlowingStarsTitle = ({
  ...
  return (
    <h2 
      onClick={ currentProject?.type !== 'empty' ? seeProject : () => {} }
      onMouseEnter={() => {
        setMouseEnter(true);
      }}
      onMouseLeave={() => {
      ...
      className={cn("font-bold text-2xl inline-block text-[#eaeaea] cursor-pointer transition-colors duration-300 hover:text-blue-300 ", className)}>
      {children}
    </h2>
  );
};
