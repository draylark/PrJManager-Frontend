...
export const Commit = () => {
    ...
    const fetchCommitDiff = async () => {
        try {
            const { data: { diff, branches } } = await axios.get(`${backendUrl}/commits/${repository.repoID}/diff/${commitHash}`, {
                headers: {
  ...
  return (
    ...
    <div className='relative flex flex-col space-y-1 items-center  w-full  h-full overflow-y-auto overflow-x-hidden '>
        ...
        <div className='flex flex-col flex-grow max-h-[88v] w-[96%] overflow-y-auto py-4 space-y-4'>
            {
                isLoading
                ? ( 
                    <div className='flex flex-grow h-full items-center justify-center border-t-[1px] border-black'>
