...
import { TextField, Button, ListItem, ListItemText, Select, MenuItem, Dialog, DialogTitle, DialogContent, DialogActions, DialogContentText, SelectChangeEvent } from '@mui/material';
...
interface ProjectCollaboratorsFormProps {
    setIsProjectCollaboratorsFormOpen: React.Dispatch<React.SetStateAction<boolean>>;
    isProjectCollaboratorsFormOpen: boolean;
    showOptModal: boolean;
    setShowOptModal: React.Dispatch<React.SetStateAction<boolean>>;
}

interface CFromServer extends Omit<CollaboratorBase, 'repository' | 'layer' | 'project'> {
  project: {
    accessLevel: string;
  };
}
...
export const ProjectCollaboratorsForm: React.FC<ProjectCollaboratorsFormProps> = ({ 
  setIsProjectCollaboratorsFormOpen, isProjectCollaboratorsFormOpen, showOptModal, setShowOptModal }) => {

    // const dispatch = useDispatch();      
    const location = useLocation();
    const { ID } = location.state.project;
    const { uid } = useSelector((stateU: RootState) => stateU.auth); 
    const { currentProject } = useSelector((stateP: RootState) => stateP.platypus);
    ...
    const handleChangeAccessLevel = (
      ...
      const updatedProjectCollaborators = form.values.collaborators.map((collab, collaboratorIndex) =>
      ...
      // Si el nivel de acceso es igual al nivel de acceso original, elimina el colaborador modificado
                                  ...
                                  <ListItemAvatar>
                                    ...
                                    />
                                    {
                                      ...
                                      <Select
                                          ...
                                          onChange={e => handleChangeAccessLevel( e, collaborator, index, form, currentCollaborators )}
                                        >
                                        <MenuItem value="contributor">Contributor</MenuItem>
                                        <MenuItem value="coordinator">Coordinator</MenuItem>
                                        <MenuItem value="administrator">Administrator</MenuItem>
                                        {
                                          currentProject && currentProject.owner === uid &&
                                            <MenuItem 
                                              disabled 
                                              value="Major">
                                                Major
                                    ...
                                    <RemoveCircleOutlineOutlined 
                                      ...
                                      className="w-5 h-5 ml-4 text-red-500 hover:text-red-700 transition-colors duration-300 cursor-pointer" onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}                                                                              />
                                  </ListItem>
                              ))}
                              </div>
                              )}
